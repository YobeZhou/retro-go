...
#define SPI_BUFFER_LENGTH     (4 * RG_SCREEN_HEIGHT) // In pixels (uint16)
...
#define ILI9341_CMD(cmd, data...) {const uint8_t x[] = data; ili9341_cmd(cmd, x, sizeof(x));}
	...
	usleep(100 * 1000);
#elif RG_SCREEN_TYPE == 5  // st7796s 100ask
	ILI9341_CMD(0xCF, {0x00,0x83,0x30});  //65k mode
	ILI9341_CMD(0xED, {0x64,0x03,0x12,0x81}); //VCOM
	ILI9341_CMD(0xE8, {0x85,0x01,0x79}); //Display Rotation
	ILI9341_CMD(0xCB, {0x39,0x2c,0x00,0x34,0x02});  //Porch Setting
	ILI9341_CMD(0xF7, {0x20});  //Gate Control //12.2v   -10.43v
	ILI9341_CMD(0xEA, {0x00,0x00});  //VCOM
	ILI9341_CMD(0xC0, {0x26});  //Power control
	ILI9341_CMD(0xC1, {0x11});  //VDV and VRH Command Enable
	ILI9341_CMD(0xC5, {0x35,0x3e});  //VRH Set 4.3+( vcom+vcom offset+vdv)
	ILI9341_CMD(0xC7, {0xBE});  //VDV Set 0v
    ILI9341_CMD(0x36, {0x28});
    ILI9341_CMD(0x3A, {0x55});
    ILI9341_CMD(0xB1, {0x00,0x1B});
    ILI9341_CMD(0xF2, {0x08});
    ...
    ILI9341_CMD(0x36, {0x48,0x88,0x28,0xE8});
